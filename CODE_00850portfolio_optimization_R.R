...
df_long_weights <- df_all_weights %>%
  ...
  left_join(data.frame(Stock = STOCKS, Name = STOCK_NAMES), by = "Stock") %>%
  mutate(Label = paste0(Stock, "\n", Name)) %>%
  filter(Weight > 0.001)   # 過濾趨近於 0 的雜訊，讓圖表更乾淨

p8_weights <- ggplot(df_long_weights,
                     aes(x = format(Date, "%Y-%m"), y = Weight, fill = Label)) +
  geom_bar(stat = "identity", position = "stack",
           color = "white", linewidth = 0.2) +
  facet_wrap(~ Strategy, ncol = 1) +
  scale_y_continuous(labels = percent) +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  labs(title  = "各策略每期持股權重變化（2018–2025）",
       subtitle = "每半年依優化結果動態調整，僅顯示權重 > 0.1% 的持股",
       x    = "期別",
       y    = "權重配置",
       fill = "股票代號") +
  theme_minimal() +
...
ggsave(file.path(PLOT_DIR, "08_weights_history.png"), p8_weights, width = 14, height = 12, dpi = 300)
