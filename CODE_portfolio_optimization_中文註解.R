...
relative_perf <- results_wealth %>%
  ...
  mutate(across(-Date, ~ . / Benchmark)) %>%
  select(-Benchmark) %>%  # 大盤自己除以自己 = 1，不需要顯示
...
p6_relative <- ggplot(relative_perf, aes(x = Date, y = Relative_Value, color = Strategy)) +
  ...
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  labs(title = "相對績效走勢（策略 / 大盤）",
       subtitle = "大於 1.0 表示優於大盤，小於 1.0 表示落後大盤",
...
# 匯出 CSV
write.csv(performance_summary, file.path(OUTPUT_DIR, "performance_summary_with_benchmark.csv"), row.names = FALSE)
write.csv(alpha_df, file.path(ANALYSIS_DIR, "alpha_analysis.csv"), row.names = FALSE)
write.csv(ir_df, file.path(ANALYSIS_DIR, "information_ratio.csv"), row.names = FALSE)
write.csv(wr_df, file.path(ANALYSIS_DIR, "win_rate_analysis.csv"), row.names = FALSE)
write.csv(win_loss_matrix, file.path(ANALYSIS_DIR, "period_by_period_results.csv"), row.names = FALSE)
write.csv(risk_return_data, file.path(ANALYSIS_DIR, "risk_return_profile.csv"), row.names = FALSE)
